...
int ftpSendFileToSocket(int sd, const char *const filename) {
    ...
    do {
        ...
        if (!ftpSendDataToSocket(sd, (char *) sbuf, len)) {
            fclose(fil);
            return (0);
        }
    } while (len == 16384);
...
int ftpconnect() {
    ...
    for (timout = ftptimeout;
         connect(sd, (struct sockaddr *) &socketAddress, sizeof(socketAddress)) != 0 && timout > 0; timout -= 1000)
        msleep(1000);
    if (timout <= 0) {
    ...
    if ((dataSocket = socket(PF_INET, SOCK_STREAM, 0)) == -1) {
        if (debugflag > 1) puts("Unable to open socket");
        return 0;
    }
