...
void WriteMKZY(int speclen,long *spec,short channel)
{
  ...
  if(mwrite((char *)&MKZY,sizeof(struct MKZYhdr),(char *)sbuf,outsiz)==0)
    if(debugflag) syslog(LOG,"Not enough RAM memory to store work.pak file!\nYou must reduce nr of measrements in cfg.txt");
...
void SendMotor(int power)
{
  ...
  if(!timout)
    { if(debugflag) syslog(LOG,"SendMotor 2 timeout."); return; }
}
  ...
  if(debugflag>0)
    {
      char m[8];
      strcpy(txt,"ADC:");      
      for(i=0;i<8;i++) { sprintf(m," %04d",adout[i]); strcat(txt,m); }
      syslog(LOG,txt);
    }
  SetTaskPrio(TASKPRIO_LOW);
...
void dopulses(short node0,short node1)
{
  ...
  if(motorswap) { short node=node0; node0=node1; node1=node; }
...
int GetLogData(unsigned char *gpstxt,int wait)
{
  ...
  I2Dinput();
  I2C(0);
  for(timout=wait/100;!I2D() && timout;timout--)
    { msleep(100); }

  TaskScheduling(0);
  I2C(1);
  if(!timout)
    { TaskScheduling(1); if(debugflag>0) syslog(LOG,"GetLogData timeout. Data not low"); return(0); }
  else
    {
      for(i=0;i<62;i++)
        {            
...
int SendMode(int mode)
{
  ...
  I2C(1);
  if(!timout)
    { if(debugflag) syslog(LOG,"SendMode timeout. Data not low"); return(-1); }
